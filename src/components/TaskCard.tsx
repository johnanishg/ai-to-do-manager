...
import { Edit2, Trash2, Check, Circle, Clock, AlertCircle, Flag, Calendar, AlertTriangle } from 'lucide-react';
...
const TaskCard = ({ task, onToggle, onToggleSubtask, onEdit, onDelete }: TaskCardProps): JSX.Element => {
    ...
    <div className="bg-white rounded-lg shadow-sm border p-4 mb-4 hover:shadow-md transition-shadow">
      ...
      <div className="ml-10">
            ...
            <ul className="space-y-1">
              {task.subtasks.map((subtask: Subtask) => (
                <li key={subtask.id} className="flex items-center gap-2 text-sm">
                  <button
                    onClick={() => task._id && onToggleSubtask(task._id, subtask.id)}
        ...
        <div className="flex flex-wrap gap-2 mt-3">
          ...
          {task.dueDate && (() => {
            const dueDateStatus = getDueDateStatus(task.dueDate, currentTime);
            const icon = getDueDateIcon(dueDateStatus.status);
            return (
              <span className={`px-2 py-1 rounded-full text-xs font-medium flex items-center gap-1 border ${dueDateStatus.color}`}>
                {icon}
                {dueDateStatus.text}
              </span>
            );
